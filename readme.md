...
Тут мы сначала сохраняем виджет (заметьте, что при такой архитектуре клиентом может быть не только виджет но и любой специально подготовленный яваскрипт-объект), вызываем функцию обновления его данных, сохраняем ядро в реестре и самое интересное - запускаем асинхронную задачу обновления данных.
